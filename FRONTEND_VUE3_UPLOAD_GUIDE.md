...
当前后端采用“创建任务 -> 分片上传 -> 完成合并 -> 轮询进度”的流程。
...
### 5.2 前端不要把后端 `uploadedChunks` 当作精确恢复依据

当前后端只累计已上传分片数，不记录每个 `chunkIndex` 是否已经上传。如果同一个分片重复上传，`uploadedChunks` 可能重复累加。

因此当前前端建议：

- 本轮上传进度用浏览器本地分片进度计算。
- 页面刷新后不做断点续传恢复。
- 如果后续需要断点续传，需要后端新增“查询已上传分片列表”接口。
...
1. 查询已上传分片列表，用于断点续传。
2. 分片 MD5 或 SHA-256 校验，避免传输损坏。
3. 取消上传任务接口，用于清理未完成分片。
4. 后台导入 `totalFiles`，用于展示导入百分比。
5. WebSocket 或 SSE 推送进度，替代轮询。
